...
foreach ($files as $file)
{
	...
	print "Parsing data12/$file\n"; flush();
	
	$ged = file_get_contents('data12/'.$file);
	$ged = s3(ext($ged, '/ged\(\){return "([^"]+)/'));
	preg_match_all('/<tr class=\"tr\d\".*?>(.*?)<\/tr>/', $ged, $mc);
	$mo = ext($ged, '/ri: \d{1,2}\.(\d{2})\.2012/');
	...
	for ($i = 0; $i < count($mc[1]); $i += 2)
	{
		...
		$s = new stdClass;
		$s->name     = trim2(preg_replace('/(?:^|\s)([A-Z])(?:\s|$)/', ' \1. ', str_replace(' - ', '-', ucwords(preg_replace('/\.([a-z])[\.\s]\s*/i', '. \1. ', str_replace('<br>', '', strtolower(preg_replace('/([A-Z])\s*\-\s*([A-Z])/', '\1 - \2', $mk[1][0]))))))));
